          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-8 w-8 rounded-lg" key={userData?.avatar || 'default'}>
                <AvatarImage key={`nav-${userData?.avatar || 'default-img'}`} src={getAvatarURL(userData?.avatar)} alt={userData?.email} />
          ...
          >
            <DropdownMenuLabel className="p-0 font-normal">
              <div className="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                <Avatar className="h-8 w-8 rounded-lg" key={userData?.avatar || 'default'}>
                  <AvatarImage key={`dropdown-${userData?.avatar || 'default-img'}`} src={getAvatarURL(userData?.avatar)} alt={userData?.email} />
                  <AvatarFallback className="rounded-lg font-bold">{userData?.email?.charAt(0)}</AvatarFallback>
                </Avatar>
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold">{userData?.email}</span>
