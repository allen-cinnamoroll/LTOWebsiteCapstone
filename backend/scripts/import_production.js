...
const importProductionData = async (jsonFilePath, clearExisting = false) => {
  try {
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      try {
        ...
        // Create driver document using upsert to handle duplicates
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      try {
        ...
        // Create vehicle document using upsert to handle duplicates
...
if (!jsonFilePath) {
  console.log('Usage: node import_production.js <path-to-json-file> [--clear]');
  console.log('Example: node import_production.js json/merged_2-4-3-5-11.json');
  console.log('Example: node import_production.js json/merged_2-4-3-5-11.json --clear');
  console.log('');
  console.log('Options:');
  console.log('  --clear    Clear existing data before import (optional)');
  process.exit(1);
}

// Run import
importProductionData(jsonFilePath, clearExisting);
